...
{
    public partial class FrmMantenimientoComputoPreventivo : Form
    {
        ...
        public FrmMantenimientoComputoPreventivo()
        {
            InitializeComponent();
        }

        private void FrmMantenimientoComputo_Load(object sender, EventArgs e)
        {
            conexion = new OleDbConnection(ConexionBase.conectar());
            adaptador = new OleDbDataAdapter("SELECT fecha, mantenimiento, proveedor, descripcion_actividades_realizadas, fecha_proximo_mto, observaciones, codigo_equipo, nota FROM MANTENIMIENTO_COMPUTO WHERE mantenimiento = 'PREVENTIVO' ORDER BY fecha DESC", conexion);
            datos = new DataSet();
