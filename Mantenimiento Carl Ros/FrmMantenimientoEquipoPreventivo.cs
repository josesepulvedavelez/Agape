...
{
    public partial class FrmMantenimientoEquipoPreventivo : Form
    {   
        ...
        public FrmMantenimientoEquipoPreventivo()
        {
            InitializeComponent();
        }

        private void FrmMantenimientoEquipoPreventivo_Load(object sender, EventArgs e)
        {
            conexion = new OleDbConnection(ConexionBase.conectar());
            adaptador = new OleDbDataAdapter("SELECT fecha, mantenimiento, proveedor, descripcion_actividades_realizadas, fecha_proximo_mto, observaciones, codigo_inventario, nota FROM MANTENIMIENTO_EQUIPO WHERE mantenimiento = 'PREVENTIVO' ORDER BY fecha DESC", conexion);
            datos = new DataSet();
