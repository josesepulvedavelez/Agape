...
{
    public partial class FrmMantenimientoComputoCorrectivo : Form
    {
        ...
        public FrmMantenimientoComputoCorrectivo()
        {
            InitializeComponent();
        }

        private void FrmMantenimientoComputoCorrectivo_Load(object sender, EventArgs e)
        {
            conexion = new OleDbConnection(ConexionBase.conectar());
            adaptador = new OleDbDataAdapter("SELECT fecha, mantenimiento, proveedor, descripcion_actividades_realizadas, fecha_proximo_mto, observaciones, codigo_equipo, nota FROM MANTENIMIENTO_COMPUTO WHERE mantenimiento = 'CORRECTIVO' ORDER BY fecha DESC", conexion);
            datos = new DataSet();
